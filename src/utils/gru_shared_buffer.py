...
class GRUSharedReplayBuffer(object):
    """
    ...
    def __init__(self, args, num_agents, obs_space, cent_obs_space, act_space, env_name):
        ...
        self.dict_keys = None
        if type(obs_shape[-1]) == dict:
            obs_shape = obs_shape[-1]
            share_obs_shape = share_obs_shape[-1]
            self.dict_keys = obs_shape.keys()
        ...
        if self.dict_keys == None:
            ...
            for key in self.dict_keys:
                self.share_obs[key] = np.zeros((self.episode_length + 1, self.n_rollout_threads, num_agents, *share_obs_shape[key]),
                                    dtype=np.float32)
                self.obs[key] = np.zeros((self.episode_length + 1, self.n_rollout_threads, num_agents, *obs_shape[key]), dtype=np.float32)
